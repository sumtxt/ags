...
The tables below shows the annual percentage share of today's districts (n=401) and municipalities (n=11007) for which spatial interpolation is necessary to construct a time series. More specifically, the table shows the percentage share of units for which the average area-based interpolation weight is not exactly 1 in every year since 1990. 
...
d <- ags:::xd19 %>% 
  ...
  group_by(year_xw) %>% 
  summarize(Districts=(sum(n)/n())*100)

m <- ags:::xm19 %>% 
  group_by(year_xw,ags19) %>% 
  summarize(n=mean(size_conv)<1) %>% 
  group_by(year_xw) %>% 
  summarize(Municipalities=(sum(n)/n())*100)

df <- full_join(d,m,by=c("year_xw")) %>% 
  rename(Year=year_xw)
...
The main function in the package is `xwalk_ags`. To illustrate how this function works, we use federal election data from the Saxony. The dataset that comes with the package and includes the number of voters and valid votes on the district level between 1994 and 2017. The figure below shows turnout trends based on the data. There is a notable break between 2005 and 2009 when the number of districts in Saxony declined from 29 and 13 and all district identifiers changed. 

```{r}
data(btw_sn)
ggplot(btw_sn, aes(year, valid/voters, group=district)) + 
  geom_line() + ylab("Turnout") + xlab("Year")
```

To construct a continuous time series on the number of voters and votes, we use the function `xwalk_ags`. It takes the input `data`, joins the crosswalk `xwalk` with the data by `ags` and `time` and then proceeds with area or population interpolation depending on the parameter `weight`.  If `variables` are left undefined the merged dataset will be returned alone. 
...
```{r}
ggplot(btw_sn_ags20, aes(year, valid/voters, group=ags20)) + 
  geom_line() + ylab("Turnout") + xlab("Year") + xlim(c(1994,2017))
``` 

The output provides information on the number of excluded observations as well as the number of matched and unmatched observations. Unmatched observations are the consequence of `ags` values that appear at least once in the crosswalk but not in the year when they are used in the data. More specifically, the 29 unmatched observations reported above are from the 1994 election which was preceded by a territorial reform in August 1994 reducing the number of districts from 54 to 29. The 29 new district identifiers only appear in the 1995 crosswalk for the first time.
